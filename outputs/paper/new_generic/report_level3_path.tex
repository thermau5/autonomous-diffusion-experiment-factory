...
search exhaustively (free-node optimization on 1-RF), the closed form is
statistically indistinguishable from the \emph{FID-optimal} schedule --- not
merely better than the default; and it transfers across datasets --- on
FFHQ-64 it beats the Karras default at every budget with the CIFAR-validated
exponents unchanged. All FID is Clean-FID 10k, 3 seeds (FFHQ: EDM's FID
...
Table~\ref{tab:dominance}); a free-node search on 1-RF cannot beat it, so it
is statistically the \emph{FID-optimal} schedule, not just better than
default; and it transfers to FFHQ-64 with no re-tuning, winning every budget.
\end{itemize}
...
Heun$^\dagger$ & \textbf{13.41}/338 & \textbf{5.69}/45.07 & \textbf{4.77}/11.64 & \textbf{4.48}/5.56 & \textbf{4.41}/\textbf{4.45} & \textbf{4.41}/\textbf{4.40}\\
...
resting where it started. The closed form is thus statistically
indistinguishable from the FID-optimum of the entire free-node schedule
space --- upgrading the claim from ``beats the default'' to ``is optimal.''
(A brute-force DP optimum of a discretized pixel-truncation surrogate
...
carries to a new dataset (FFHQ-64) with no re-tuning.
Path-straightening,
